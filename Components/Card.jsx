  ...
  return (
    <div className="flex justify-center">
      {props?.recommendCard ? (
        ...
        <Link href={`/discover/movie/${props?.movie?.id}`}>
          <CardContent {...props} />
        </Link>
      )}
    </div>
...
const CardContent = ({ movie, hoverCard, setHoverCard, recommendCard }) => {
  return (
    <div
        ...
        >
          ...
          {recommendCard && (
            <Link href={`/discover/movie/${movie?.id}`}>
              <button className="flex items-center justify-center w-full px-2 py-1 mt-1 text-xs font-bold text-black duration-150 rounded-lg bg-violet-400 md:text-sm">
                Info
              </button>
            </Link>
          )}
